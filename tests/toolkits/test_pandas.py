...
from pydantic_ai_toolkits.toolkits.pandas import PandasToolkit  # noqa: E402
...
@pytest.fixture
def tk(sample_df: pd.DataFrame) -> PandasToolkit:
    return PandasToolkit(dataframes={"sales": sample_df})


class TestRegistry:
    def test_empty_construction(self) -> None:
        assert PandasToolkit().list_dataframes() == []

    def test_list_dataframes(self, tk: PandasToolkit) -> None:
        entries = tk.list_dataframes()
        assert entries == [{"name": "sales", "rows": 5, "columns": ["country", "price", "qty"]}]

    def test_unknown_dataframe_raises(self, tk: PandasToolkit) -> None:
...
class TestAnalysisOps:
    def test_head_caps_at_preview(self, sample_df: pd.DataFrame) -> None:
        tk = PandasToolkit(dataframes={"x": sample_df}, max_preview_rows=2)
        assert len(tk.head("x", n=99)) == 2

    def test_describe(self, tk: PandasToolkit) -> None:
        out = tk.describe("sales")
        assert "price" in out and "qty" in out

    def test_schema(self, tk: PandasToolkit) -> None:
        s = tk.schema("sales")
        assert s["rows"] == 5
        names = {c["name"] for c in s["columns"]}
        assert names == {"country", "price", "qty"}

    def test_query(self, tk: PandasToolkit) -> None:
        rows = tk.query("sales", "price > 20")
        assert {r["country"] for r in rows} == {"DE", "FR"}

    def test_query_columns_projection(self, tk: PandasToolkit) -> None:
    ...
    def test_query_cap(self, sample_df: pd.DataFrame) -> None:
        tk = PandasToolkit(dataframes={"x": sample_df}, max_query_rows=2)
        assert len(tk.query("x", "qty >= 0")) == 2

    def test_aggregate(self, tk: PandasToolkit) -> None:
    ...
    def test_aggregate_rejects_unknown_agg(self, tk: PandasToolkit) -> None:
        with pytest.raises(ValueError, match="Unsupported aggregation"):
            tk.aggregate("sales", group_by=["country"], column="price", agg="bogus")

    def test_value_counts(self, tk: PandasToolkit) -> None:
