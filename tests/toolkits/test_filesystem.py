...
class TestReadFile:
    ...
    def test_read_too_large(self, tmp_workspace: Path) -> None:
        ...
        tk = FilesystemToolkit(root=tmp_workspace, max_bytes=100)
...
class TestWriteOps:
    def test_write_requires_writable(self, tmp_workspace: Path) -> None:
        tk = FilesystemToolkit(root=tmp_workspace, read_only=True)
        with pytest.raises(PermissionError):
            tk.write_file("new.txt", "content")

    def test_write_creates_parents(self, tmp_workspace: Path) -> None:
        tk = FilesystemToolkit(root=tmp_workspace, read_only=False)
...
class TestGlob:
    ...
    def test_glob_respects_limit(self, tmp_workspace: Path) -> None:
        ...
        tk = FilesystemToolkit(root=tmp_workspace, max_glob_results=5)
