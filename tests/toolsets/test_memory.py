...
from pydantic_ai_toolbox.toolsets.memory import (
...
def make_toolset(tmp_path: Path, **overrides) -> MemoryToolset:
    """Construct a MemoryToolset using `tmp_path` for storage by default."""
    params: dict = {
...
class TestAddMessage:
    ...
    def test_id_format_deterministic(self, tmp_path: Path) -> None:
        tk = make_toolset(tmp_path, namespace="abc")
    ...
    def test_eviction_by_max_messages(self, tmp_path: Path) -> None:
        tk = make_toolset(tmp_path, max_messages=3)
        for i in range(5):
    ...
    def test_eviction_by_max_chars(self, tmp_path: Path) -> None:
        tk = make_toolset(tmp_path, max_messages=100, max_chars=10)
    ...
    def test_both_caps_bind_simultaneously(self, tmp_path: Path) -> None:
        tk = make_toolset(tmp_path, max_messages=2, max_chars=100)
        for i in range(5):
...
class TestRetrieval:
    ...
    def test_get_recent_defaults_to_window(self, tmp_path: Path) -> None:
        tk = make_toolset(tmp_path, window=3)
        for i in range(5):
            tk.add_message("user", f"m{i}")
...
class TestSummary:
    ...
    def test_summary_shape(self, tmp_path: Path) -> None:
        tk = make_toolset(tmp_path, namespace="ns")
        tk.add_message("user", "hello")
...
class TestPersistence:
    ...
    def test_save_then_load_via_new_instance(self, tmp_path: Path) -> None:
        ...
        tk1 = MemoryToolset(storage_path=path, namespace="ns", autosave=False)
        tk1.add_message("user", "first")
        tk1.set_fact("k", 42)
        tk1.store.save()  # explicit (autosave off)
        assert path.exists()
        tk2 = MemoryToolset(storage_path=path, namespace="ns")
    ...
    def test_autosave_roundtrip(self, tmp_path: Path) -> None:
        path = tmp_path / "mem.json"
        tk1 = MemoryToolset(storage_path=path, namespace="ns", autosave=True)
    ...
    def test_atomic_write_tmp_gone(self, tmp_path: Path) -> None:
        path = tmp_path / "mem.json"
        tk = MemoryToolset(storage_path=path, autosave=True)
...
class TestNowSeam:
    ...
    def test_now_callable_used(self, tmp_path: Path) -> None:
        tk = make_toolset(tmp_path, now_fn=lambda: 1700000000.0)
