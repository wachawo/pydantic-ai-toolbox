...
### Added
- `SystemToolset`: read-only host information for agents — `cpu_info`,
  `memory_info`, `disk_usage` (with a cross-platform `used_percent`
  computed from total/free instead of psutil's macOS-misleading
  `percent`), `disk_partitions`, `uptime`, `load_avg`, `top_processes`
  (capped by `max_processes`), `network_io`, `battery`. New optional
  extra `[system]` (psutil), included in `[all]`.
